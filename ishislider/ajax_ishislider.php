...
include_once('ishislider.php');

$home_slider = new IshiSlider();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'ishislider_slides` SET `position` = '.(int)$position.'
			WHERE `id_ishislider_slides` = '.(int)$id_slide
		);
